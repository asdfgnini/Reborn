...
Q_PROPERTY_CREATE_Q_CPP(TaoAppBar, bool, IsStayTop)
Q_PROPERTY_CREATE_Q_CPP(TaoAppBar, bool, IsDefaultClosed)
Q_PROPERTY_CREATE_Q_CPP(TaoAppBar, bool, IsOnlyAllowMinAndClose)

TaoAppBar::TaoAppBar(QWidget* parent)
    : QWidget{parent}, d_ptr(new TaoAppBarPrivate())
{
...
#ifdef Q_OS_WIN
#if (QT_VERSION >= QT_VERSION_CHECK(6, 5, 3) && QT_VERSION <= QT_VERSION_CHECK(6, 8, 1))
    ...
    //标题
    d->_titleLabel = new TaoText(this);
    ...
    connect(d->_closeButton, &TaoIconButton::clicked, d, &TaoAppBarPrivate::onCloseButtonClicked);
...
void TaoAppBar::setAppBarHeight(int height)
{
    Q_D(TaoAppBar);
...
int TaoAppBar::getAppBarHeight() const
{
    Q_D(const TaoAppBar);
    return d->_pAppBarHeight;
}

void TaoAppBar::setCustomWidget(TaoAppBarType::CustomArea customArea, QWidget* widget)
{
    Q_D(TaoAppBar);
...
QWidget* TaoAppBar::getCustomWidget() const
{
    Q_D(const TaoAppBar);
    return d->_pCustomWidget;
}

void TaoAppBar::setCustomWidgetMaximumWidth(int width)
{
    Q_D(TaoAppBar);
...
int TaoAppBar::getCustomWidgetMaximumWidth() const
{
    Q_D(const TaoAppBar);
    return d->_pCustomWidgetMaximumWidth;
}

void TaoAppBar::setIsFixedSize(bool isFixedSize)
{
    Q_D(TaoAppBar);
...
bool TaoAppBar::getIsFixedSize() const
{
    Q_D(const TaoAppBar);
    return d->_pIsFixedSize;
}

void TaoAppBar::setWindowButtonFlag(TaoAppBarType::ButtonType buttonFlag, bool isEnable)
{
    Q_D(TaoAppBar);
    if (isEnable)
    {
...
void TaoAppBar::setWindowButtonFlags(TaoAppBarType::ButtonFlags buttonFlags)
{
    Q_D(TaoAppBar);
    ...
    d->_maxButton->setVisible(d->_buttonFlags.testFlag(TaoAppBarType::MaximizeButtonHint));
    d->_closeButton->setVisible(d->_buttonFlags.testFlag(TaoAppBarType::CloseButtonHint));
}

TaoAppBarType::ButtonFlags TaoAppBar::getWindowButtonFlags() const
{
...
int TaoAppBar::takeOverNativeEvent(const QByteArray& eventType, void* message, qintptr* result)
#else
int TaoAppBar::takeOverNativeEvent(const QByteArray& eventType, void* message, long* result)
#endif
{
    Q_D(TaoAppBar);
    ...
    case WM_LBUTTONDBLCLK:
    {
        ...
        postData.insert("WMClickType", TaoAppBarType::WMLBUTTONDBLCLK);
        TaoEventBus::getInstance()->post("WMWindowClicked", postData);
        return 0;
    }
    case WM_LBUTTONDOWN:
    {
        QVariantMap postData;
        postData.insert("WMClickType", TaoAppBarType::WMLBUTTONDOWN);
        TaoEventBus::getInstance()->post("WMWindowClicked", postData);
        return 0;
    }
    case WM_LBUTTONUP:
    {
        QVariantMap postData;
        postData.insert("WMClickType", TaoAppBarType::WMLBUTTONUP);
        TaoEventBus::getInstance()->post("WMWindowClicked", postData);
        return 0;
    }
    ...
    {
        QVariantMap postData;
        postData.insert("WMClickType", TaoAppBarType::WMNCLBUTTONDOWN);
        TaoEventBus::getInstance()->post("WMWindowClicked", postData);
...
bool TaoAppBar::eventFilter(QObject* obj, QEvent* event)
{
    Q_D(TaoAppBar);
    switch (event->type())
    {
