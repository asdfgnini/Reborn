...
TaoComboBoxStyle::TaoComboBoxStyle(QStyle* style)
{
...
TaoComboBoxStyle::~TaoComboBoxStyle()
{
}

void TaoComboBoxStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
...
void TaoComboBoxStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    ...
    {
        //container区域
        if (widget->objectName() == "TaoComboBoxContainer")
        {
            QRect viewRect = option->rect;
            painter->save();
            painter->setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing);
            tTheme->drawEffectShadow(painter, viewRect, _shadowBorderWidth, 6);
...
void TaoComboBoxStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    switch (control)
    {
    case QStyle::CC_ComboBox:
    {
        ...
        if (const QStyleOptionComboBox* copt = qstyleoption_cast<const QStyleOptionComboBox*>(option))
        {
            ...
            painter->setBrush(isEnabled ? copt->state.testFlag(QStyle::State_MouseOver) ? TaoThemeColor(_themeMode, BasicHover) : TaoThemeColor(_themeMode, BasicBase) : TaoThemeColor(_themeMode, BasicDisable));
...
QRect TaoComboBoxStyle::subControlRect(ComplexControl cc, const QStyleOptionComplex* opt, SubControl sc, const QWidget* widget) const
{
    switch (cc)
    {
    case QStyle::CC_ComboBox:
...
QSize TaoComboBoxStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
