...
TaoMenuBarStyle::TaoMenuBarStyle(QStyle* style)
{
...
TaoMenuBarStyle::~TaoMenuBarStyle()
{
}

void TaoMenuBarStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
...
void TaoMenuBarStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    case QStyle::CE_ToolButtonLabel:
    {
        if (const QStyleOptionToolButton* topt = qstyleoption_cast<const QStyleOptionToolButton*>(option))
        {
            ...
            //展开图标
            painter->setPen(!topt->state.testFlag(QStyle::State_Enabled) ? Qt::gray : TaoThemeColor(_themeMode, BasicText));
            QFont iconFont = QFont("TaoAwesome");
            iconFont.setPixelSize(18);
            painter->setFont(iconFont);
            painter->drawText(topt->rect, Qt::AlignCenter, QChar((unsigned short)TaoIconType::AngleRight));
            painter->restore();
        }
        return;
    }
    ...
    case QStyle::CE_MenuBarItem:
    {
        if (const QStyleOptionMenuItem* mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option))
        {
            ...
            const TaoMenuBar* menuBar = dynamic_cast<const TaoMenuBar*>(widget);
            if (menuBar)
            {
                QAction* action = menuBar->actionAt(menuItemRect.center());
                if (action)
                {
                    iconText = action->property("TaoIconType").toString();
                }
            }
            if (menuText.isEmpty())
            {
                ...
                {
                    painter->save();
                    painter->setPen(!mopt->state.testFlag(QStyle::State_Enabled) ? TaoThemeColor(_themeMode, BasicTextDisable) : TaoThemeColor(_themeMode, BasicText));
                    QFont iconFont = QFont("TaoAwesome");
...
QSize TaoMenuBarStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
...
int TaoMenuBarStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
    switch (metric)
    {
