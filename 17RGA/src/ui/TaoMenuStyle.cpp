...
TaoMenuStyle::TaoMenuStyle(QStyle* style)
{
    _pMenuItemHeight = 32;
...
TaoMenuStyle::~TaoMenuStyle()
{
}

void TaoMenuStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    case QStyle::PE_PanelMenu:
    {
        ...
        tTheme->drawEffectShadow(painter, option->rect, _shadowBorderWidth, 6);
        // 背景绘制
...
void TaoMenuStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    case QStyle::CE_MenuItem:
    {
        ...
        if (const QStyleOptionMenuItem* mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option))
        {
            if (mopt->menuItemType == QStyleOptionMenuItem::Separator)
            {
                ...
                painter->setBrush(TaoThemeColor(_themeMode, BasicBaseLine));
            ...
            else
            {
                ...
                if (mopt->state.testFlag(QStyle::State_Enabled) && (mopt->state.testFlag(QStyle::State_MouseOver) || mopt->state.testFlag(QStyle::State_Selected)))
                {
                    ...
                    painter->setPen(Qt::NoPen);
                    painter->setBrush(TaoThemeColor(_themeMode, PopupHover));
                ...
                {
                    painter->save();
                    painter->setPen(!mopt->state.testFlag(QStyle::State_Enabled) ? Qt::gray : _themeMode == TaoThemeType::Light ? Qt::black
                                                                                                                                : Qt::white);
                ...
                {
                    QString iconText;
                    const TaoMenu* menu = dynamic_cast<const TaoMenu*>(widget);
                    if (menu)
                    {
...
int TaoMenuStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
    switch (metric)
    {
...
QSize TaoMenuStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
