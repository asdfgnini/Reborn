...
TaoMicaBaseInitObject::TaoMicaBaseInitObject(TaoApplicationPrivate* appPrivate, QObject* parent)
    : QObject{parent}
{
    _appPrivate = appPrivate;
}

TaoMicaBaseInitObject::~TaoMicaBaseInitObject()
{
}

void TaoMicaBaseInitObject::onInitMicaBase(QImage img)
{
    // QColorDialog
    // 统一处理为1920*1080以节省空间
    img = img.scaled(QSize(1920, 1080), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

    QImage blurImage = TaoExponentialBlur::doExponentialBlur(img, 500).toImage();
