...
TaoNavigationStyle::TaoNavigationStyle(QStyle* style)
{
    _pOpacity = 1;
    _pItemHeight = 40;
...
TaoNavigationStyle::~TaoNavigationStyle()
{
}

void TaoNavigationStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
...
void TaoNavigationStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    ...
    case QStyle::CE_ItemViewItem:
    {
        if (const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option))
        {
            ...
            TaoNavigationNode* node = static_cast<TaoNavigationNode*>(vopt->index.internalPointer());
            TaoNavigationModel* model = dynamic_cast<TaoNavigationModel*>(const_cast<QAbstractItemModel*>(vopt->index.model()));
            ...
            // 图标绘制
            painter->setPen(vopt->index == _pPressIndex ? TaoThemeColor(_themeMode, BasicTextPress) : TaoThemeColor(_themeMode, BasicText));
...
QSize TaoNavigationStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
...
void TaoNavigationStyle::navigationNodeStateChange(QVariantMap data)
{
    if (data.contains("Expand"))
    {
        TaoNavigationNode* lastExpandNode = _expandAnimationTargetNode;
        _opacityAnimationTargetNode = data.value("Expand").value<TaoNavigationNode*>();
    ...
    {
        TaoNavigationNode* lastExpandNode = _expandAnimationTargetNode;
        _opacityAnimationTargetNode = data.value("Collapse").value<TaoNavigationNode*>();
        _expandAnimationTargetNode = _opacityAnimationTargetNode;
        _pOpacity = 0;
...
bool TaoNavigationStyle::_compareItemY(TaoNavigationNode* node1, TaoNavigationNode* node2)
{
    ...
    {
        TaoNavigationNode* node1OriginalNode = node1->getOriginalNode();
        TaoNavigationNode* node2OriginalNode = node2->getOriginalNode();
        // 不同父节点  相同起源节点
        if (node1OriginalNode == node2OriginalNode)
        {
            ...
            {
                TaoNavigationNode* node1ParentNode = node1->getParentNode();
                TaoNavigationNode* node2ParentNode = node2->getParentNode();
