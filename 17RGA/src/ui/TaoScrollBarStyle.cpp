...
TaoScrollBarStyle::TaoScrollBarStyle(QStyle* style)
{
    _pIsExpand = false;
    _pOpacity = 0;
    _pScrollBar = nullptr;
...
TaoScrollBarStyle::~TaoScrollBarStyle()
{
}

void TaoScrollBarStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    ...
    case QStyle::CC_ScrollBar:
    {
        if (const QStyleOptionSlider* sopt = qstyleoption_cast<const QStyleOptionSlider*>(option))
        {
            ...
            if (_pIsExpand)
            {
                ...
                int sideLength = 8;
                painter->setBrush(TaoThemeColor(_themeMode, ScrollBarHandle));
...
int TaoScrollBarStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
...
int TaoScrollBarStyle::styleHint(StyleHint hint, const QStyleOption* option, const QWidget* widget, QStyleHintReturn* returnData) const
{
...
void TaoScrollBarStyle::startExpandAnimation(bool isExpand)
{
    if (isExpand)
    {
        _pIsExpand = true;
