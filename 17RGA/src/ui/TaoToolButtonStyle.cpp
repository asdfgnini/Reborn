...
TaoToolButtonStyle::TaoToolButtonStyle(QStyle* style)
{
    _pIsSelected = false;
...
TaoToolButtonStyle::~TaoToolButtonStyle()
{
}

void TaoToolButtonStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    switch (control)
    {
    case QStyle::CC_ToolButton:
    {
        ...
        if (const QStyleOptionToolButton* bopt = qstyleoption_cast<const QStyleOptionToolButton*>(option))
        {
            ...
            painter->setPen(_pIsTransparent ? Qt::transparent : TaoThemeColor(_themeMode, BasicBorder));
            // 背景绘制
            if (bopt->state.testFlag(QStyle::State_Enabled))
            {
                if (bopt->state.testFlag(QStyle::State_Sunken))
                {
                    painter->setBrush(_pIsTransparent ? TaoThemeColor(_themeMode, BasicPressAlpha) : TaoThemeColor(_themeMode, BasicPress));
                ...
                else
                {
                    if (_pIsSelected)
                    {
                        painter->setBrush(_pIsTransparent ? TaoThemeColor(_themeMode, BasicSelectedAlpha) : TaoThemeColor(_themeMode, BasicHover));
                        painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                    }
                    else
                    {
                        ...
                        {
                            painter->setBrush(_pIsTransparent ? TaoThemeColor(_themeMode, BasicHoverAlpha) : TaoThemeColor(_themeMode, BasicHover));
                            painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                        }
                        else
                        {
...
QSize TaoToolButtonStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
...
void TaoToolButtonStyle::_drawIndicator(QPainter* painter, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
...
void TaoToolButtonStyle::_drawIcon(QPainter* painter, QRectF iconRect, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    if (bopt->toolButtonStyle != Qt::ToolButtonTextOnly)
    {
        QSize iconSize = bopt->iconSize;
        if (widget->property("TaoIconType").toString().isEmpty())
        {
            // 绘制QIcon
        ...
        else
        {
            ...
            {
                painter->setPen(TaoThemeColor(_themeMode, BasicTextDisable));
            }
            QFont iconFont = QFont("TaoAwesome");
            switch (bopt->toolButtonStyle)
            {
            case Qt::ToolButtonIconOnly:
            {
                iconFont.setPixelSize(0.75 * std::min(iconSize.width(), iconSize.height()));
                painter->setFont(iconFont);
                painter->drawText(iconRect, Qt::AlignCenter, widget->property("TaoIconType").toString());
                break;
            }
            case Qt::ToolButtonFollowStyle:
            case Qt::ToolButtonTextBesideIcon:
            {
                ...
                iconFont.setPixelSize(0.75 * std::min(iconSize.width(), iconSize.height()));
                painter->setFont(iconFont);
                painter->drawText(adjustIconRect, Qt::AlignCenter, widget->property("TaoIconType").toString());
                break;
            }
            case Qt::ToolButtonTextUnderIcon:
            {
                ...
                painter->setFont(iconFont);
                painter->drawText(adjustIconRect, Qt::AlignHCenter, widget->property("TaoIconType").toString());
                break;
            }
            default:
            {
...
void TaoToolButtonStyle::_drawText(QPainter* painter, QRect contentRect, const QStyleOptionToolButton* bopt) const
{
...
qreal TaoToolButtonStyle::_calculateExpandIndicatorWidth(const QStyleOptionToolButton* bopt, QPainter* painter) const
{
    // 展开指示器
    QSize iconSize = bopt->iconSize;
    painter->save();
...
void TaoToolButtonStyle::drawItemPixmap(QPainter *painter, const QRect &rect, int alignment, const QPixmap &pixmap)
{
    // 可以先调用基类的实现，如果没有特殊需求
    QProxyStyle::drawItemPixmap(painter, rect, alignment, pixmap);

    // 如果你需要自定义绘制逻辑，可以在这里添加
}
