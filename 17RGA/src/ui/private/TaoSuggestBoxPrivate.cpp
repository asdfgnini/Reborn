...
TaoSuggestion::TaoSuggestion(QObject* parent)
    : QObject(parent)
{
    _pTaoIcon = TaoIconType::None;
    _pSuggestText = "";
    _pSuggestData = QVariantMap();
}

TaoSuggestion::~TaoSuggestion()
{
}

TaoSuggestBoxPrivate::TaoSuggestBoxPrivate(QObject* parent)
    : QObject{parent}
{
}

TaoSuggestBoxPrivate::~TaoSuggestBoxPrivate()
{
}

void TaoSuggestBoxPrivate::onThemeModeChanged(TaoThemeType::ThemeMode themeMode)
{
    _themeMode = themeMode;
    _searchEdit->removeAction(_themeMode == TaoThemeType::Light ? _darkSearchAction : _lightSearchAction);
    _searchEdit->addAction(_themeMode == TaoThemeType::Light ? _lightSearchAction : _darkSearchAction, QLineEdit::TrailingPosition);
    _searchEdit->update();
}

void TaoSuggestBoxPrivate::onSearchEditTextEdit(const QString& searchText)
{
    Q_Q(TaoSuggestBox);
    ...
    QVector<TaoSuggestion*> suggestionVector;
...
void TaoSuggestBoxPrivate::onSearchViewClicked(const QModelIndex& index)
{
    Q_Q(TaoSuggestBox);
    _searchEdit->clear();
    ...
    TaoSuggestion* suggest = _searchModel->getSearchSuggestion(index.row());
    Q_EMIT q->suggestionClicked(suggest->getSuggestText(), suggest->getSuggestData());
    _startCloseAnimation();
}

void TaoSuggestBoxPrivate::_startSizeAnimation(QSize oldSize, QSize newSize)
{
...
void TaoSuggestBoxPrivate::_startExpandAnimation()
{
...
void TaoSuggestBoxPrivate::_startCloseAnimation()
{
