...
QString HttpClientPrivate::readReply(QNetworkReply *reply, const QString &charset) {
    ...
    // Qt6 及以上使用 QString::fromUtf8() 或其他方法进行字符编码转换
    if (charset == "UTF-8") {
        result = QString::fromUtf8(byteArray);  // 默认 UTF-8
    } else {
        // 若需要其他字符集，可以使用 QLocale 或 QString::fromLocal8Bit
        result = QString::fromLocal8Bit(byteArray);  // 这是一个常见的本地编码，具体根据 charset 进行扩展
    }
#else
    // Qt5 使用 QTextStream 和 setCodec
    QTextStream in(&byteArray, QIODevice::ReadOnly);
    in.setCodec(charset.toUtf8());  // 设置字符编码
    result = in.readAll();  // 读取所有数据
#endif
