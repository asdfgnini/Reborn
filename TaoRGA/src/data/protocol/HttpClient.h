...
private:
    ...
    // HttpClient client("http://192.168.1.101/mmsp/measurement/binaryData/get?@start=200747");

    // client.header("Content-Type", "application/octet-stream")
    //         .success([](const QString &response){
    //         QByteArray binaryData = response.toUtf8(); // 同样假设response是QString，这里转换为QByteArray
    //         qDebug() << "Response received, size: " << binaryData.size() << " bytes";
    //         QFile file("receivedData.bin");
    //         if (file.open(QIODevice::WriteOnly)) {
    //             file.write(binaryData);
    //             file.close();
    //             qDebug() << "Binary data saved to receivedData.bin";
    //         } else {
